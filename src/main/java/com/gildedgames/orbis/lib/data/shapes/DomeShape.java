...
public class DomeShape extends AbstractShape
{
	...
	public DomeShape(final BlockPos start, final BlockPos end, final boolean centered)
	{
		this.start = start;
		this.end = end;
	...
	{
		return new DomeShape(this.start.add(x, y, z), this.end.add(x, y, z), this.centered);
	}
	...
	public boolean contains(final int x, final int y, final int z)
	{
		...
		{
			final BlockPos center = RegionHelp.getCenter(this.getBoundingBox()).add(0, -this.getBoundingBox().getHeight() / 2, 0);
		...
		else
		{
			...
			final BlockPos center = RegionHelp.getCenter(new Region(this.start, newEnd))
					.add(0, -this.getBoundingBox().getHeight() / 2, 0);
