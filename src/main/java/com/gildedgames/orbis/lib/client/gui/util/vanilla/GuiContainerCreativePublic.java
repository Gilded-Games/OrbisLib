...
public class GuiContainerCreativePublic extends InventoryEffectRenderer
{
	...
	public GuiContainerCreativePublic(EntityPlayer player)
	{
		super(null);

		this.inventorySlots = new GuiContainerCreativePublic.ContainerCreativePublic(player, this);
		...
		else
		{
			...
			else if (selectedTabIndex == CreativeTabs.INVENTORY.getIndex())
			{
				...
				else if (type == ClickType.THROW && slotIn != null && slotIn.getHasStack())
				{
					...
					this.mc.playerController.sendSlotPacket(itemstack1, ((GuiContainerCreativePublic.CreativeSlotPublic) slotIn).slot.slotNumber);
				}
				...
				else
				{
					this.mc.player.inventoryContainer
							.slotClick(slotIn == null ? slotId : ((GuiContainerCreativePublic.CreativeSlotPublic) slotIn).slot.slotNumber, mouseButton, type,
									this.mc.player);
			...
			else if (type != ClickType.QUICK_CRAFT && slotIn.inventory == basicInventory)
			{
				...
				if (!itemstack5.isEmpty() && !itemstack7.isEmpty() && itemstack5.isItemEqual(itemstack7) && ItemStack
						.areItemStackTagsEqual(itemstack5, itemstack7))
				{
					if (mouseButton == 0)
					{
						if (flag)
			...
			else if (this.inventorySlots != null)
			{
				...
				else if (slotIn != null)
				{
					...
					this.mc.playerController
							.sendSlotPacket(itemstack4, slotIn.slotNumber - this.inventorySlots.inventorySlots.size() + 9 + this.extraSlots + 36);
	...
	public void initGui()
	{
		if (this.mc.playerController.isInCreativeMode())
		{
			...
			if (tabCount > 12)
			{
				...
				this.maxPages = (int) Math.ceil((tabCount - 12) / 10D);
			}
		}
		else
		{
	...
	{
		GuiContainerCreativePublic.ContainerCreativePublic guicontainercreative$containercreative = (GuiContainerCreativePublic.ContainerCreativePublic) this.inventorySlots;
		...
		if (tab.hasSearchBar() && tab != CreativeTabs.SEARCH)
		{
			...
			if (!this.searchField.getText().isEmpty())
			{
				...
				while (itr.hasNext())
				{
					...
					for (String line : stack.getTooltip(this.mc.player,
		...
		else
		{
			guicontainercreative$containercreative.itemList
					.addAll(this.mc.getSearchTree(SearchTreeManager.ITEMS).search(this.searchField.getText().toLowerCase(Locale.ROOT)));
		}
	...
	/**
	 * returns (if you are not on the inventoryTab) and (the flag isn't setDataParent) and (you have more than 1 page of items)
	 */
	private boolean needsScrollBars()
	{
		...
		return selectedTabIndex != CreativeTabs.INVENTORY.getIndex() && CreativeTabs.CREATIVE_TAB_ARRAY[selectedTabIndex].hasScrollbar()
				&& ((GuiContainerCreativePublic.ContainerCreativePublic) this.inventorySlots).canScroll();
	}
	...
	private void setCurrentCreativeTab(CreativeTabs tab)
	{
		...
		if (tab == CreativeTabs.INVENTORY)
		{
			...
			{
				Slot slot = new GuiContainerCreativePublic.CreativeSlotPublic(container.inventorySlots.get(l), l);
	...
	public void handleMouseInput() throws IOException
	{
		...
		{
			int j = (((GuiContainerCreativePublic.ContainerCreativePublic) this.inventorySlots).itemList.size() + 9 - 1) / 9 - 5;
			...
			((GuiContainerCreativePublic.ContainerCreativePublic) this.inventorySlots).scrollTo(this.currentScroll);
		}
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks)
	{
		...
		if (this.destroyItemSlot != null && selectedTabIndex == CreativeTabs.INVENTORY.getIndex() && this
			...
			for (int i = 0; i < 5; ++i)
			{
				for (int j = 0; j < 9; ++j)
				{
					this.addSlotToContainer(
							new GuiContainerCreativePublic.LockedSlot(GuiContainerCreativePublic.basicInventory, i * 9 + j, 9 + j * 18, 18 + i * 18));
				}
			}
		...
		public void scrollTo(float pos)
		{
			...
			for (int k = 0; k < 5; ++k)
			{
				for (int l = 0; l < 9; ++l)
				{
					...
					{
						GuiContainerCreativePublic.basicInventory.setInventorySlotContents(l + k * 9, this.itemList.get(i1));
					}
					else
					{
	...
	public static class CreativeSlotPublic extends Slot
	{
		...
		public CreativeSlotPublic(Slot p_i46313_2_, int index)
		{
